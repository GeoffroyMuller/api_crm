...
const serviceFactory = <T extends Model, AuthModel extends Model | unknown = unknown>(model: ModelClass<T>, opts?: ServiceFactoryOptions<T, AuthModel>): Service<T, AuthModel> => {
  ...
  const getById = async (id: ID, auth: any, relations?: RelationExpression<T>[], filters?: any) => {
  ...
  return {
    ...
    onBeforeUpdate,
    getById,
    getAll: async (relations: RelationExpression<T>[], filters: any, auth: any) => {
      const query = model.query();
      if (Array.isArray(relations)) {
        for (const relation of relations) {
          query.withGraphFetched(relation);
    ...
    paginate: async (relations: RelationExpression<T>[], filters: any, auth: any) => {
      const query = model.query();
      if (Array.isArray(relations)) {
        for (const relation of relations) {
          query.withGraphFetched(relation);
